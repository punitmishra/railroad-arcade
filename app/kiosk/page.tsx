    ...
    <div className="fixed inset-0 bg-[#0a0a0f] overflow-hidden select-none" onClick={resetIdle}>
      ...
      {kioskState === 'playing' && (
        <div className="h-full flex">
          {/* Camera View (left side - 60%) */}
          <div className="w-[60%] h-full">
            <KioskCameraView showControls={true} />
          </div>

          {/* Controls (right side - 40%) */}
          <div className="w-[40%] h-full p-4 overflow-y-auto bg-[#0a0a0f]/50">
            <div className="space-y-4">
              {/* Token Display */}
              <div className="flex items-center justify-between p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
                <span className="text-amber-400 font-medium">TOKENS</span>
                <span className="text-2xl font-bold text-amber-400 font-mono">{tokens}</span>
              </div>
...
function AttractScreen({ tokens, onStart }: AttractScreenProps) {
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-[#0a0a0f] via-[#12121c] to-[#0a0a0f]">
      ...
      <div className="relative z-10 text-center">
        ...
        <div className="mb-8">
          <h1
            className="text-6xl md:text-8xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-purple-400 to-cyan-400 animate-gradient"
            style={{ fontFamily: 'Orbitron, sans-serif' }}
          >
            RAILROAD
          </h1>
          <h2
            className="text-4xl md:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-cyan-400 to-purple-400"
            style={{ fontFamily: 'Orbitron, sans-serif' }}
          >
            ARCADE
          </h2>
        ...
        {/* Token display */}
        {tokens > 0 && (
          <div className="mb-8 px-6 py-3 rounded-xl bg-amber-500/20 border border-amber-500/30 inline-block">
            <span className="text-amber-400 text-2xl font-bold font-mono">{tokens} TOKENS</span>
          </div>
        )}
        ...
        <div className="mb-12">
          <button
            onClick={onStart}
            className="px-12 py-6 rounded-2xl bg-gradient-to-r from-cyan-500 to-purple-500 text-white text-2xl font-bold hover:opacity-90 transition-opacity shadow-lg shadow-cyan-500/20 animate-bounce"
            style={{ fontFamily: 'Orbitron, sans-serif' }}
          >
            PRESS START
          </button>
          <p className="mt-4 text-gray-500">or press ENTER</p>
        </div>

        {/* Features highlight */}
        <div className={`transition-opacity duration-500 ${showHighlights ? 'opacity-100' : 'opacity-0'}`}>
          <div className="flex flex-wrap justify-center gap-4 max-w-2xl">
            {['Multi-Level Track', 'Real-Time Control', 'Game Modes', 'Leaderboards'].map((feature) => (
              <span
                key={feature}
